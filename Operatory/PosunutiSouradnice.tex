...
\begin{solution}
	...
	\item
		...
		\end{equation}
		\begin{note}
			První rovnosti v~\eqref{eq:TranslationOperatorX} lze rozumět jako definici operátoru $\operator{T}(a)$ na prostoru vlnových funkcí v $x$ reprezentaci, podobně jako
			\begin{equation}
				\matrixelement{x}{\operator{p}}{\psi}
					=-\im\hbar\derivative{}{x}\psi(x)\equiv\operator{p}\psi(x).
			\end{equation}
		\end{note}
	\end{enumerate}
\end{solution}
...
\begin{note}
	Stejným způsobem lze definovat i unitární operátor posunutí hybnosti\footnote{Znaménko v exponentu je zde opačné než u operátoru $\operator{T}(a)$.}\index{operátor!posunutí hybnosti}
	\begin{equation}
		\label{eq:MomentumTranslationOperator}
		\operator{P}(b)
			=\e^{+\frac{\im}{\hbar}b\operator{x}},
	\end{equation}
	který splňuje
	\begin{equation}
		\operator{P}^{-1}(b)\,\operator{p}\,\operator{P}(b)
			=\operator{p}+b.
	\end{equation}
