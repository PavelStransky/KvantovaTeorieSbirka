...
\note
    ...
    % \begin{enumerate}
    % \item
    %     Napište Heisenbergovy rovnice pohybu pro operátory $\operator{a}(t)$ a $\operator{a}^{\dagger}(t)$.
    %     Najděte jejich obecné řešení.

    % \item
    %     Předpokládejte, že v čase $t=0$ je oscilátor v základním stavu $\ket{0}$.
    %     Nalezněte vlnovou funkci oscilátoru v čase $t>0$.
    %     Ukažte, že tato vlnová funkce je koherentní stav.
        
    % \item
    %     Spočítejte střední hodnoty polohy $\matrixelement{\psi(t)}{\operator{x}}{\psi(t)}$ a hybnosti $\matrixelement{\psi(t)}{\operator{p}}{\psi(t)}$ oscilátoru v čase $t>0$.
    %     Porovnejte je s klasickými rovnicemi pohybu harmonického oscilátoru pod vlivem vnější síly.
