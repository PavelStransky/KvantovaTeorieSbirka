...
\exercise[14.5.2024]{Interakce způsobená nerozlišitelností částic}
    ...
    \end{equation*}
    kde $\sigma$ určuje určuje šířku balíku.
    ...
    \begin{enumerate}
        \item 
            ...
            Vlnová funkce může být symetrická nebo antisymetrická. 
        ...
        \item Spočítejte efektivní sílu
            ...
            \end{equation*}
            Bude tato síla přitažlivá nebo odpudivá a jak závisí na typu nerozlišitelných částic (symetrická nebo antisymetrická prostorová část vlnové funkce)?
            Nakreslete graf závislosti $F(b)$ pro oba případy.
        \end{enumerate}
...
\exercise{Kvartický oscilátor}
    ...
    \item
        ...
        \item 
            ...
            Velikost matice ať je taková, aby tyto hladiny byly určeny s přesností alespoň na pět platných cifer.
        \item 
            Vysvětlete, proč jsou dvojice hladin $(E_{0}, E_{1})$ a $(E_{2}, E_{3})$ v téměř degenerovaných dubletech?        
        \item 
            Opakujte výpočet pro jiné hodnoty $\hbar$, například $\hbar=0.1$ a $\hbar=0.5$, a diskutujte, jaký vliv má volba $\hbar$ na výsledné spektrum.
            Nezapomeňte pokaždé zkontrolovat konvergenci energetických hladin (obecně pro menší hodnoty $\hbar$ je potřeba více bázových stavů).
        \end{itemize}		
...
\exercise{Základní stav atomu helia variačně}
% https://chem.libretexts.org/Courses/Pacific_Union_College/Quantum_Chemistry/08%3A_Multielectron_Atoms/8.02%3A_Perturbation_Theory_and_the_Variational_Method_for_Helium
Hamiltonián popisující elektrony obalu atomu helia je
\begin{equation*}
    \operator{H}=\frac{\vectoroperator{p}_{1}^{2}}{2M}
        +\frac{\vectoroperator{p}_{2}^{2}}{2M}
        -\frac{2\gamma}{\operator{r}_{1}}
        -\frac{2\gamma}{\operator{r}_{2}}
        +\frac{\gamma}{\abs{\vectoroperator{r}_{1}-\vectoroperator{r}_{2}}},\qquad \gamma=\frac{e^{2}}{4\pi\epsilon_{0}}\,,
\end{equation*}
kde první dva členy jsou kinetická energie elektronů, druhé dva členy popisují interakci jednotlivých elektronů s atomovým jádrem a poslední člen udává elektrostatické odpuzování mezi oběma elektrony.
Elektrony v obalu jsou nerozlišitelné fermiony se spinem $s=\frac{1}{2}$ a hmotností $M$.
Jádro považujte za bodovou částici, spin-orbitální interakci a jaderné pohyby zanedbejte.

Uvažujte následující normalizovanou testovací vlnovou funkci pro jeden elektron základního stavu helia:
\begin{equation*}
    \psi(\vector{r};Z_\ti{eff})
        =\frac{1}{\sqrt{\pi}}\left(\frac{Z_{\ti{eff}}}{a_{0}}\right)^{\frac{3}{2}}\e^{-\frac{Z_{\ti{eff}}}{a_{0}}r}\,,
\end{equation*}
kde $a_{0}=\hbar^{2}/(\gamma M)$ je Bohrův poloměr a $Z_\ti{eff}>0$ je variační parametr představující efektivní náboj jádra, který elektron vnímá.\footnote{Pro $Z_\ti{eff}=1$ by tato funkce odpovídala vlnové funkci základního stavu elektronu v atomu vodíku.}

\begin{enumerate}
    \item \emph{(1/2 bodu)}
        Napište prostorovou část testovací vlnové funkce $\Psi(\vector{r}_{1},\vector{r}_{2};Z_\ti{eff})$ pro oba elektrony helia v základním stavu.
        Jak splníte antisymetrizační požadavek na celkovou vlnovou funkci fermionů?

    \item \emph{(2 body)}
        Vypočítejte analyticky střední hodnotu energie
        \begin{equation*}
            E(Z_\ti{eff})
                =\matrixelement{\Psi}{\operator{H}}{\Psi}
                =\int\Psi^{*}(\vector{r}_{1},\vector{r}_{2};Z_\ti{eff})\operator{H}\Psi(\vector{r}_{1},\vector{r}_{2};Z_\ti{eff})\d^{3}\vector{r}_{1}\d^{3}\vector{r}_{2}\,.    
        \end{equation*}
    
        Využijte znalosti integrálu přes úhlové proměnné:
        \begin{align*}
        I(r_{1},r_{2})
            &=\int\frac{\d\Omega_{1}\d\Omega_{2}}{\abs{\vector{r}_{1}-\vector{r}_{2}}}
            =4\pi\int_{0}^{2\pi}\d\phi_{2}\int_{0}^{\pi}\sin{\theta}\d\theta\frac{1}{\sqrt{r_{1}^{2}+r_{2}^{2}-2r_{1}r_{2}\cos\theta}}
            =\frac{\left(4\pi\right)^2}{\max(r_{1},r_{2})}
        \end{align*}
    ...
    \item \emph{(1 bod)}
        Najděte optimální hodnotu variačního parametru $Z^*_\ti{eff}$ a odpovídající energii.

    \item \emph{(1/2 bodu)}
        Porovnejte získaný odhad energie základního stavu helia s experimentální hodnotou $E_{\ti{exp}}=-79,\!0\,\mathrm{eV}$ a určete relativní chybu pro zadanou testovací funkci.

    \item \emph{(1/2 bodu)}
        Jaký je fyzikální význam získané hodnoty variačního parametru? Jaká je jeho hodnota ve srovnání s nábojem jádra helia $Z=2$ a proč?

    \item \emph{(1/2 bodu)}
        Navrhněte jinou testovací vlnovou funkci pro elektrony helia, která by mohla sloužit k odhadu energie základního stavu.
        Zdůvodněte svůj návrh.

    \item \emph{(nepovinně)}
        Vypočítejte střední hodnotu energie základního stavu helia pomocí vámi navržené testovací vlnové funkce a porovnejte získaný odhad s výsledkem z předchozích bodů.

    \end{enumerate}

\note
    Úlohu můžete řešit v atomových jednotkách ($\hbar=M=\gamma=1$), ve kterých Bohrův poloměr je $a_{0}=1$ a energie je měřena v jednotkách Hartree ($1\,E_\ti{H}=2\unit{Ry}=2\cdot13\c6\unit{eV}$); v těchto jednotkách je $E_{\ti{exp}}=-2\c90\,E_\ti{H}$.
    
\exercise{Koherentní stavy v Kerrově oscilátoru}
\setcounter{footnote}{0}
    Nelineární Kerrův oscilátor je popsán Hamiltoniánem
    \begin{equation*}
        \operator{H}
            =\hbar\Omega\,\operator{a}^{\dagger}\operator{a}+\frac{\hbar K}{2}\left(\operator{a}^{\dagger}\operator{a}\right)^{2},
    \end{equation*}
    kde $\Omega$, $K$ jsou reálné kladné parametry a operátory $\operator{a}$, $\operator{a}^{\dagger}$ jsou posunovací operátory harmonického oscilátoru splňující komutační relaci $[\operator{a},\operator{a}^{\dagger}]=1$.
    V čase $t=0$ je oscilátor připraven v koherentním stavu $\ket{z}$.

    \begin{enumerate}
        \item \emph{(1/2 bodu)}
            Nalezněte spektrum operátoru $\operator{H}$ a jeho vlastní vektory.
            Využijte toho, že znáte spektrum lineárního harmonického oscilátoru $\operator{H}_{0}$=$\hbar\Omega\,\left(\operator{a}^{\dagger}\operator{a}+1/2\right)$, který má vlastní energie $E_{n}^{(0)}=\hbar\Omega\,\left(n+1/2\right)$ a vlastní vektory $\ket{n}$, $n=0,1,2,\dotsc$.

        \item \emph{(1/2 bodu)}
            Dokažte, že charakteristická funkce\footnote{
            Charakteristická funkce náhodné veličiny s rozdělením pravděpodobnosti $P_n$ je definovaná jako
            \begin{equation*}
                G_{P}(\theta)\equiv\left\langle \e^{\im\theta n}\right\rangle_{P}=\sum_{n}P_{n}\e^{\im\theta n}.
            \end{equation*}
            Je to vlastně Fourierova transformace hustoty pravděpodobnosti a plně určuje rozdělení pravděpodobnosti a všechny momenty dané náhodné veličiny:
            \begin{equation*}
                \left\langle n^{k}\right\rangle_{P}
                    =\left.\frac{1}{\im^{k}}\frac{\d^{k}G_{P}(\theta)}{\d\theta^{k}}\right|_{\theta=0}.
            \end{equation*}
        } Poissonova rozdělení $P_{n}=\lambda^{n}e^{-\lambda}/n!$ je
        \begin{equation*}
            G_{P}(\theta)=\exp\left[\lambda\left(\e^{\im\theta}-1\right)\right].
        \end{equation*}

        \item\emph{(1 bod)}
            Nalezněte explicitní vztah ve tvaru funkce (nikoliv řady) pro střední hodnotu $\left\langle\operator{a}(t)\right\rangle$.
            Využijte výsledku z předchozího bodu a skutečnosti, že střední hodnota počtu kvant oscilátoru v koherentním stavu $\ket{z}$ je $\left\langle \operator{a}^{\dagger}\operator{a}\right\rangle=\abs{z}^{2}$.

        \item\emph{(1/2 bodu)}
            Nalezněte střední hodnotu souřadnice a hybnosti v Kerrově oscilátoru v čase $t>0$.

        \item\emph{(1 bod)} 
            Předpokládejte, že nelinearita Kerrova oscilátoru je malá, tj. $K\ll\Omega$.
            Rozviňte {\color{red} vnořenou exponenciálu} vypočítaného výrazu pro $\left\langle\operator{a}(t)\right\rangle$ do druhého řádu v parametru $K$ a diskutujte fyzikální význam získaného výsledku.

        \item\emph{(1/2 bodu)}
            Ve výsledku předchozího bodu byste měli získat Gaussovské potlačení oscilací s časem, $\left\langle\operator{a}(t)\right\rangle\propto e^{-\alpha t^2}$.
            Vypočítejte \emph{čas rozpadu} $t_{\ti{decay}}$ jako čas, za který amplituda oscilací poklesne na hodnotu $1/e$. 
            Diskutujte platnost tohoto výsledku.

        \item\emph{(1/2 bodu)}
            Určete \emph{čas znovuoživení} $t_{\ti{revival}}$ jako čas, za který se amplituda $\left\langle\operator{a}(t)\right\rangle$ vrátí na svou počáteční velikost.
            Diskutujte vztah časů $t_{\ti{decay}}$ a $t_{\ti{revival}}$ v závislosti na parametrech modelu.

        \item\emph{(1/2 bodu)}
            Vykreslete grafy $\abs{\left\langle\operator{a}(t)\right\rangle}$ a $\langle\operator{x}(t)\rangle$ pro vhodně zvolené parametry.

    \end{enumerate}

\note
    Kerrův oscilátor je důležitý model v kvantové optice, který popisuje chování světla v nelineárních optických prostředích.
    Efektivně také modeluje anharmonicitu transmon qubitů (založených na Josephsonově přechodu) v supravodivých obvodech používaných v kvantových počítačích.

\exercise{Částice na kruhovém řetízku}
Uvažujte uzavřený řetízek s $L>2$ místy, na kterých se může nacházet částice (viz obrázek).
Stav takové částice můžeme popsat lineární kombinací bázových stavů $\ket{j}_{j=1}^L$, kde stav $|j\rangle$ popisuje částici sedící na $j$-tém místě řetízku.
...
\begin{tikzpicture}[scale=2]
    ...
    \foreach \k in {0,...,7} {
        \coordinate (P\k) at ({cos(45*\k)},{sin(45*\k)});
        \fill (P\k) circle (1.5pt);
    }

    % Label first four sites (P0,P1,P2,P3)
    % Shift the label slightly outward in the radial direction
        % Labels for the first four sites, shifted slightly outward
    \node at (1.25, 0.1) {$|\dots\rangle$};                           % P0
    \node at ({1.15*cos(45)},{1.15*sin(45) + 0.07}) {$|3\rangle$}; % P1
    \node at (0, 1.22) {$|2\rangle$};                              % P2
    \node at ({-1.15*cos(45)},{1.15*sin(45) + 0.07}) {$|1\rangle$};% P3

\end{tikzpicture}
\end{figure}

Dále uvažujte, že částice na řetízku může skočit vždy na sousední místo. 
Takové chování lze modelovat pomocí následujícího hamiltoniánu
\begin{equation*}
    \operator{H} = e\sum_{j = 1}^L\projector{j} + \nu\sum_{j = 1}^L \left(\ket{j}\bra{j+1} + \ket{j+1}\bra{j}\right),
\end{equation*}
kde první člen je potenciál ($e$ odpovídá energii, kterou má částice za to, že sedí na $j$-tém místě) a druhý člen --- \emph{hopping} --- je kinetický ($\nu^{-1}$ škáluje kinetickou energii podobně jako hmotnost částice).
Uzavřenost řetízku odpovídá periodické okrajové podmínce $\ket{j+L} =\ket{j}$.

\begin{enumerate}
    \item \emph{(1 bod)} Najděte normalizované vlastní stavy a energie hamiltoniánu $\hat{H}$. Celý proces výpočtu důkladně okomentujte.
    Řešení byste měli nalézt ve formě rovinných vln 
    \begin{equation*}
        \ket{k_a} \equiv \frac{1}{\sqrt{L}} \sum_{j=1}^L e^{\frac{2\pi i}{L}k_aj}|j\rangle.
    \end{equation*}
    Jaká je fyzikální interpretace parametru $k_a$ a jakých hodnot může nabývat?
    
    \textit{Nápověda: Jeden z možných způsobů je řešení pomocí diferenční rovnice. Přepište hamiltonián na matici v bázi stavů $\{\ket{j}\}_{j=1}^L$ a řešte vlastní rovnici $\operator{H}\ket{h}=E_h\ket{h}$ pro vlastní stavy $\ket{h}$ a energie $E_h$ jako diferenční rovnici s anzatzem pro elementy vlastního vektoru $h_j\equiv h^j$. 
    Využijte periodické okrajové podmínky $h_{j+L} = h_j$.}

   \item \emph{(1 bod)} Určete základní stav v bázi $\ket{j}$ stavů a základní energii v závislosti na hodnotách a znaménku parametrů $e,\nu \in \mathbb{R}$ a délce řetízku $L$.
\end{enumerate}

Dále se omezte na $\nu < 0$. 

Zatím jsme uvažovali jen jednu částici na řetízku. Můžeme ale přidat další. Hamiltonián systému $N$ nerozlišitelných částic ($N$ neinteragujících částic běhajících po řetízku), kde každá je popsaná hamiltoniánem $\operator{H}\equiv\operator{H}^{(j)}$, lze zapsat jako 
\begin{equation*}
    \operator{H}_N = \operator{H}^{(1)}\otimes\operator{1}^{(2)}\otimes\dots\otimes\operator{1}^{(N)} + \operator{1}^{(1)}\otimes\operator{H}^{(2)}\otimes\dots\otimes\mathbb{I}_{(N)}+ \dots
\end{equation*}
\begin{enumerate}
  \setcounter{enumi}{2}
  \item (1 bod) Určete energie základních stavů 2, 3, 4 a 5-ti částicových systémů pro případ, kdy jsou částice na řetízku fermiony se spinem 1/2, a pro případ, kdy se jedná o bosony se spinem 1. Jak se budou energie lišit pro fermiony se spinem 3/2?
\end{enumerate}

V následujícím uvažujte na řetízku jen \emph{dva nerozlišitelné fermiony} se spinem $1/2$. 
Model rozšíříme o interakční člen mezi částicemi (potenciální energii)
\begin{equation*}
    \operator{H}_I = \lambda\sum_{j=1}^{L}\projector{j}^{(1)} \otimes \projector{j}^{(2)},
\end{equation*}
který modeluje srážky částic na řetízku.
Parametr $\lambda$ odpovídá energii, kterou částice získají, když se budou obě nacházet na stejném místě $\ket{j}$.
Podle znaménka parametru lze rozlišit, jestli bude interakce \uv{přitažlivá}, nebo \uv{odpudivá}.
Celkový dvoučásticový hamiltonián je
\begin{equation*}
    \operator{H}_{2} =\operator{H}^{(1)}\otimes \operator{1}^{(2)} + \operator{1}^{(1)}\otimes\operator{H}^{(2)} + \operator{H}_I.
\end{equation*}

\begin{enumerate}
  \setcounter{enumi}{3}
  \item\emph{(2 body)} Pomocí poruchové teorie určete do prvního řádu v $\lambda$ základní stav a energii dvoučásticového hamiltoniánu $\operator{H}_{2}$ jako opravu základního stavu hamiltoniánu neinteragujících částic $\operator{H}^{(1)}\otimes \operator{1}^{(2)} + \operator{1}^{(1)}\otimes \operator{H}^{(2)}$. 
  Jaký je celkový spin základního stavu neinteragujících částic a jak se změní přidáním interakčního členu? 
  
  \textit{Nápověda: Rozmyslete si, jak vypadá báze prostoru pro dvě nerozlišitelné částice. Můžete využít buď Slaterovy determinanty, nebo adaptovat bázi na singletní a tripletní spinové stavy.}

  Výsledek interpretujte pomocí jednočásticových stavů $\ket{k_{a}}$ a porovnejte se základním stavem neinteragujících částic. Můžete k tomu využít například grupovou rychlost, kterou lze pro jednočásticové stavy spočítat jako derivaci jednočásticové energie
  \begin{equation*}
    v_g(k_a) = \frac{\text{d}E(k_a)}{dk_a}.    
  \end{equation*}
\end{enumerate}

\chapter{II. 2025 -- 2026}
\exercise{Dvouhladinový systém s periodickou interakcí}
    Dvouhladinový systém je popsaný Hamiltoniánem
    \begin{align*}
        \operator{H}(t)
            &=\operator{H}_{0}+\operator{H}_{\ti{I}}(t),\\
        ...
        \operator{H}_{\ti{I}}(t)
            &\equiv \Theta(t)\makematrix{0 & \gamma\e^{\im\omega t} \\ \gamma\e^{-\im\omega t} & 0}
                =\Theta(t)\left[\gamma\e^{\im\omega t}\ket{\phi_{1}}\bra{\phi_{2}}
                    +\gamma\e^{-\im\omega t}\ket{\phi_{2}}\bra{\phi_{1}}\right],
    \end{align*}
    přičemž operátor $\operator{H}_{\ti{I}}(t)$ představuje periodickou interakci, která je zapnuta v čase $t_{0}=0$ (formálně zapsáno pomocí Heavisideovy skokové funkce $\Theta$), $\gamma$ je reálný parametr, který určuje sílu interakce, a $E_{1,2}^{(0)}$ jsou neporušené energie.

    Před zapnutím interakce v čase $t\leq0$ je systém ve stavu $\ket{\phi_{i}}\equiv\ket{\phi_{1}}=\makematrix{1\\0}$.

    \begin{enumerate}
        \item 
            Spočítejte \emph{neporuchově} pravděpodobnost $\mathcal{P}_{1\rightarrow2}(t)$, že systém v čase v čase $t>0$ přejde do stavu $\ket{\phi_{2}}$.
            Vzorec, který dostanete, se nazývá \emph{Rabiho formule}.

        \emph{Nápověda:} Pro řešení této části úlohy můžete využít výsledku pro časový vývoj spinu v harmonickém magnetickém poli, který jsme si odvodili v rámci cvičení zimního semestru.

        \item 
            Předpokládejte nyní, že interakce je slabá, tj. $\gamma\ll\abs{E_{2}^{\hi{0}}-E_{1}^{\hi{0}}}$, a řešte úlohu pomocí nestacionární poruchové teorie do druhého řádu v $\gamma$.
            Získanou pravděpodobnost srovnejte s přesným řešením.
            Za jaké podmínky přibližné řešení dobře aproximuje přesný výsledek?

        \item 
            Za jaké podmínky lze v čase $t>0$ naměřit systém ve stavu $\ket{\phi_{2}}$ s pravděpodobností jedna?
    \end{enumerate}

\exercise{Rozptyl nerozlišitelných částic}
    Interakce dvou částic s hmotností $M$ a spinem $s=\frac{1}{2}$ je popsána potenciálem
    \begin{equation*}
        V(r)=V_{0}(r)-V_{\ti{s}}(r)\vector{\sigma}\hi{1}\cdot\vector{\sigma}\hi{2}
    \end{equation*}
    ...
    \begin{enumerate}
        ...
        \item Diskutujte podrobně případ rozptylu \emph{nerozlišitelných částic}.
            Jaké účinné průřezy má smysl studovat a proč?
            Jaké budou jejich amplitudy rozptylu v rámci 1. Bornovy aproximace?
