...
\begin{solution}
	...
	\begin{subequations}
		\begin{align}
			\psi_{\mathrm{I}}(x)&=\frac{\mathcal{A}_{-}}{\sqrt{\abs{p(x)}}}\e^{-\frac{1}{\hbar}\int_{x}^{-a}\abs{p(x')}\d x'}\equiv\lambda\e^{-I_{x}^{-a}},\\
			\psi_{\mathrm{III}}(x)&=\lambda'\e^{-I_{a}^{x}},
		\end{align}				
	\end{subequations}
	kde se používá značení~\eqref{eq:WKBNotation}.
	Klasická hybnost je
	\begin{equation}
	...
	\begin{align}
		\psi_{\mathrm{II}}(x)
			=\lambda\cos{\left[I_{-a}^{x}-\frac{\pi}{4}\right]}
			=\lambda\cos{\left[I_{-a}^{a}-\left(I_{x}^{a}+\frac{\pi}{4}\right)\right]}\nonumber\\
			=\underbrace{\lambda\cos{I_{-a}^{a}}}_{0}\cos{\left[I_{x}^{a}+\frac{\pi}{4}-\right]}-\underbrace{\lambda\sin{I_{-a}^{a}}}_{\lambda'}\sin{\left[I_{x}^{a}+\frac{\pi}{4}\right]}.
	\end{align}
	Musí tedy platit
	\begin{equation}
	...
	\end{equation}
	což je Bohrova-Sommerfeldova kvantovací podmínka.\index{podmínka!Bohr-Sommerfeldova kvantovací}
	Zároveň platí, že
	\begin{align}
		\sin{I_{-a}^{a}}&=\sin\left(\pi n+\frac{\pi}{2}\right)=\minus{n}&\Longrightarrow&&\lambda'&=\minus{n}\lambda,
	\end{align}
	tj. pro sudé hodnoty kvantového čísla $n$ je vlnová funkce sudá, pro liché hodnoty je lichá.
	...
	\end{align}
	Primitivní funkce k poslednímu integrálu je
	\begin{align}
		\int\sqrt{1-y^{2}}\,\d y
	...
	\end{align}
	což je rovné \emph{přesnému} spektru harmonického oscilátoru.

	Jak vypadají vlnové funkce ve WKB aproximaci?
	V oblasti II je
	\begin{align}
		\psi_{\mathrm{II}}(x)
			...
			&=\frac{2\mathcal{A}}{\sqrt{p(x)}}\sin\left[\frac{E}{\hbar\Omega}\left(\arccos{\frac{x}{a}}-\frac{x}{a}\sqrt{1-\frac{x^{2}}{a^{2}}}\right)+\frac{\pi}{4}\right].
	\end{align}	
	Naopak v klasicky zakázaných oblastech I a III vychází integrál
	\begin{align}
		I_{x_{1}}^{x_{2}}
	...
	\begin{align}
		\psi_{\mathrm{I}}(x)
			...
			&=(-1)^{n}\frac{\mathcal{A}}{\sqrt{\abs{p(x)}}}\e^{\frac{E}{\hbar\Omega}\left[\ln{\left(-\frac{x}{a}+\sqrt{\frac{x^{2}}{a^{2}}-1}\right)}+\frac{x}{a}\sqrt{\frac{x^{2}}{a^{2}}-1}\right]}
	\end{align}
	a
	\begin{align}
		\psi_{\mathrm{III}}(x)
			&=\lambda\e^{-\frac{E}{\hbar\Omega}\left[-\arccosh{y}+x\sqrt{y^{2}-1}\right]_{1}^{\frac{x}{a}}}
			 =\frac{\mathcal{A}}{\sqrt{\abs{p(x)}}}\e^{-\frac{E}{\hbar\Omega}\left[-\arccosh{\frac{x}{a}}+\frac{x}{a}\sqrt{\frac{x^{2}}{a^{2}}-1}\right]}.
	\end{align}
	Konstanta $\mathcal{A}$ se určí z normalizace, přičemž pro $n$ velké a $\hbar=\Omega=M=1$ konverguje k hodnotě
	\begin{equation}
		\label{eq:WKBHarmonicOscillatorNormalization}
		\mathcal{A}_{-}\xrightarrow{n\gg 1}\frac{1}{\sqrt{2\pi}}.		
	\end{equation}
	...
	\begin{figure}[!htbp]
		...
		\scaption{
			...
			WKB vlnové funkce jsou normalizované faktorem~\eqref{eq:WKBHarmonicOscillatorNormalization}.
		}
		\label{fig:howf}
	\end{figure}	
