\subsection{$\alpha$ rozpad}
	Rozpad $\alpha$, například rozpad radia na radon\footnote{
		Použitá notace je ${}^{A}_{Z}\mathrm{X}$, kde $A=N+Z$ je celkový počet nukleonů nuklidu $\mathrm{X}$ \emph{(nukleonové nebo hmotnostní číslo)}\index{číslo!nukleonové}, $N$ je počet neutronů \emph{(neutronové číslo)}\index{číslo!neutronové} a $Z$ počet protonů \emph{(protonové číslo)}\index{číslo!protonové} = náboj jádra.
	}
	\begin{equation}
	...
	\end{equation}
	kde $Z$ je protonové číslo jádra produktu, $Z_{\alpha}=2$ protonové číslo $\alpha$ částice, $V_{0}$ je kladný parametr,
	...
	\begin{enumerate}
		\item 
			Ve WKB přiblížení odvoďte tzv. \emph{Gamovův koeficient průchodu}\index{koeficient průchodu!Gamovův}
			\begin{equation}
				\label{eq:Gammov}
				\important{
		...
		Pro šířku jámy použijte přibližný vztah $a=a_{0}(\sqrt[3]{A}+\sqrt[3]{A_{\alpha}})$, kde $A$ je atomové číslo jádra po rozpadu, $A_\alpha=4$ je atomové číslo $\alpha$ částice, a $a_{0}\doteq1,\!2$ fm.
		...
		\item
			...
			\begin{center}
				\begin{tabular}{|c|c|c|}
					...
					\hline
					\hline
					$^{144}_{\ 60}{\text{Nd}}$ & $1.90$ & $2\cdot10^{15}$ let\\
					\hline
					$^{224}_{\ 88}{\text{Ra}}$ & $5.79$ & $3.6$ dne\\
					\hline
					$^{212}_{\ 84}{\text{Po}}$ & $8.95$ & $0.3\,\mu$s\\
					\hline
				\end{tabular}
			\end{center}
	...
	\end{enumerate}
\begin{note}
	Podrobná diskuze různých přístupů k $\alpha$ rozpadu včetně WKB aproximace je v~\cite{Holstein1996}.
\end{note}
...
\begin{solution}
	...
	\item
		...
		\begin{align}
			\psi_{\mathrm{III}}(x)
				&=\frac{\mathcal{C}}{\sqrt{\abs{p(x)}}}\e^{\frac{\im}{\hbar}\int_{b}^{x}\abs{p(x')}\d x'-\im\frac{\pi}{4}}
		...
		\end{equation}
		přejde vlnová funkce do tvaru
		\begin{equation}
			\psi_{\mathrm{II}}(x)=\lambda F\e^{I_{a}^{x}}+\frac{\im\lambda}{2F}\e^{-I_{a}^{x}}.
		\end{equation}
		Do oblasti I se následně naváže pomocí sešívacích podmínek~\eqref{eq:WKBConnectionUp1}---\eqref{eq:WKBConnectionUp2}:
		\begin{align}
			\psi_{\mathrm{I}}(x)
				&=\lambda F\cos\left[I_{x}^{a}+\frac{\pi}{4}\right]+\frac{\im\lambda}{F}\sin\left[I_{x}^{a}+\frac{\pi}{4}\right]\nonumber\\										
				&=\underbrace{\mathcal{C}\left(F+\frac{1}{F}\right)\e^{\im\frac{\pi}{4}}}_{\mathcal{A}}\frac{\e^{\im I_{x}^{a}}}{\sqrt{\abs{p(x)}}}
					+\underbrace{\mathcal{C}\left(F-\frac{1}{F}\right)\e^{-\im\frac{\pi}{4}}}_{\mathcal{B}}\frac{\e^{-\im I_{x}^{a}}}{\sqrt{\abs{p(x)}}}.
		\end{align}
		Pravděpodobnost průchodu tedy vychází
		\begin{equation}
			T=\abs{\frac{\mathcal{C}}{\mathcal{A}}}^{2}=\frac{1}{\left(F+\frac{1}{F}\right)^{2}}\approx F^{2}\,,
		\end{equation}
		...
		Dosazení za $F$ dá hledaný Gamovův koeficient~\eqref{eq:Gammov}.
		
	\item
		Do právě odvozeného Gamovova vztahu pro pravděpodobnost průniku bariérou se dosadí modelový potenciál~\eqref{eq:AlphaDecayPotential}.
		Klasická hybnost $\alpha$ částice o energii $E_{\alpha}$ v oblasti Coulombické části potenciálu je
		\begin{equation}
			p(x)=\sqrt{2M_{\alpha}\left(E_{\alpha}-\frac{Z_{\alpha}Z\gamma}{x}\right)}.
		\end{equation}
		...
		\begin{align}
			I_{a}^{b}=\frac{1}{\hbar}\int_{a}^{b}\abs{p(x)}\d x
				&=\frac{\sqrt{2M_{\alpha}E_{\alpha}}}{\hbar}\int_{a}^{b}\sqrt{\frac{Z_{\alpha}Z\gamma}{E_{\alpha}x}-1}\,\d x=\equationcomment{u=\frac{x}{b} \\ \d x=b\,\d u}\nonumber\\
				&=\underbrace{\frac{Z_{\alpha}Z\gamma}{\hbar}\sqrt{\frac{2M_{\alpha}}{E_{\alpha}}}}_{\kappa}\int_{x=a}^{x=b}\sqrt{\frac{1}{u}-1}\,\d u=\equationcomment{u=y^{2} \\ \d u=2y\d y}\nonumber\\				
		...
		\begin{equation}
			T=\exp{\left\{\frac{2Z_{\alpha}Z\gamma}{\hbar}\sqrt{\frac{2M_{\alpha}}{E_{\alpha}}}\left[\arccos{\sqrt{\frac{a}{b}}}-\sqrt{\frac{a}{b}\left(1-\frac{a}{b}\right)}\right]\right\}}
		\end{equation}
	...
	\item
		...
		\begin{equation}
			E_{\alpha}=\frac{v_{\alpha}^{2}}{2M_{\alpha}},
		\end{equation}
		přičemž energie je dána jako rozdíl klidové hmotnosti $M_{{}^{A+4}_{Z+2}X}$ jádra vstupujícího do reakce a klidové hmotnosti produktů (jádro $M_{{}^{A}_{Z}Y}$ + jádro helia $M_{\alpha}\equiv M_{{}^{4}_{2}\mathrm{He}}$)
		\begin{equation}
			E_{\alpha}=\frac{1}{c^{2}}\left(M_{{}^{A+4}_{Z+2}X}-M_{{}^{A}_{Z}Y}-M_{\alpha}\right)\,.
		\end{equation}
		Rychlost tedy vychází
		\begin{equation}
			v_{\alpha}=\sqrt{\frac{2\left(E_{\alpha}+V_{0}\right)}{M_{\alpha}}}
		\end{equation}
		a z ní střední doba života
		\begin{equation}
			\tau=\frac{2a}{T}\sqrt{\frac{M_{\alpha}}{2\left(E_{\alpha}+V_{0}\right)}}.
		\end{equation}
	...
	\item
		...
		\begin{center}
			\begin{tabular}{|c|c|c|c|c|c|c|c|c|c|}
				...
				\hline
				\hline
				$^{144}_{\ 60}{\text{Nd}}$ & $1.90$ & $8.14$ & $90.9$ & $0.23$ & $108$ & $1.59\cdot10^{-47}$ & $1.17\cdot10^{25}$ & $3.21\cdot10^{17}$ let \\
				\hline
				$^{224}_{\ 88}{\text{Ra}}$ & $5.79$ & $9.15$ & $43.8$ & $0.24$ & $63.6$ & $2.45\cdot10^{-28}$ & $7.2\cdot10^{5}$ & $8.4$ dní \\
				\hline
				$^{212}_{\ 84}{\text{Po}}$ & $8.95$ & $9.1$ & $27.0$ & $0.24$ & $34.2$ & $1.34\cdot10^{-15}$ & $5.9\cdot10^{-4}$ & $0.129$ $\mu$s\\
				\hline
			\end{tabular}
		\end{center}
		I přes velmi hrubé aproximace a zjednodušený výpočet dostáváme řádovou shodu s naměřenými hodnotami ze zadání příkladu (až na neodym, kde je rozdíl dva řády).
		
	\item
		De Broglieovy vlnové délky $\alpha$ částic jsou
		\begin{equation}
			\lambda_{\alpha}=\frac{p_{\alpha}}{\hbar}=\frac{M_{\alpha}v_{\alpha}}{\hbar}\,.
		\end{equation}
		Číselně
		\begin{center}
			\begin{tabular}{|c|c|}
				...
				\hline
				\hline
				$^{144}_{\ 60}{\text{Nd}}$ & $0.23$ \\
				\hline
				$^{224}_{\ 88}{\text{Ra}}$ & $0.22$ \\
				\hline
				$^{212}_{\ 84}{\text{Po}}$ & $0.22$ \\
				\hline
			\end{tabular}
		\end{center}
	...
	\begin{note}
		...
		\begin{align}
			\ln\tau
				...
				&=\ln{\frac{2a_{0}\sqrt[3]{A}}{\sqrt{2\left(E_{\alpha}+V_{0}\right){M_{\alpha}}}}}
					+\frac{2Z_{\alpha}Z\gamma}{\hbar}\sqrt{\frac{2M_{\alpha}}{E_{\alpha}}}\left[\frac{\pi}{2}-2\sqrt{\frac{a_{0}}{Z_{\alpha}\gamma}}\sqrt{E_{\alpha}}A^{\frac{1}{6}}Z^{-\frac{1}{2}}\right]
		\end{align}
		a pro těžká jádra, pro která platí $A\approx 2Z$, dostaneme
		\begin{align}
			\ln\tau&=\frac{1}{3}\ln{Z}+\frac{1}{3}\ln{2}+\ln{\frac{2a_{0}}{\sqrt{\frac{2\left(E_{\alpha}-V_{0}\right)}{M_{\alpha}}}} }+\frac{2Z_{\alpha}\gamma\sqrt{2M_{\alpha}}}{\hbar}\frac{\pi}{2}\frac{Z}{\sqrt{E_{\alpha}}}
				-4\frac{\sqrt{2Z_{\alpha}\gamma a_{0}M_{\alpha}}}{\hbar}\sqrt[6]{2}Z^{\frac{2}{3}}
		\end{align}
		To je v souladu s experimentálně pozorovanou závislostí
		\begin{equation}
			(\ln{\tau})_{\mathrm{exp}}=C_{1}\left(\frac{Z}{\sqrt{E_{\alpha}}}-\zeta Z^{\frac{2}{3}}\right)-C_{2},
		\end{equation}
		která se nazývá \emph{Geigerův-Nuttallův zákon}\index{zákon!Geigerův-Nuttallův} a ve které se konstanty $C_{1,2}$ a $\zeta$ fitují z dostupných experimentálních dat~\cite{Geiger1911}.
	\end{note}
\end{solution}
