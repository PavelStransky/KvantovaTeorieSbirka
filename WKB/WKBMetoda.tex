...
\begin{equation}
    \psi(x)=\frac{1}{\sqrt{\abs{p(x)}}}\left[\mathcal{A}_{+}\e^{\frac{\im}{\hbar}\int_{x_{0}}^{x}\abs{p(x')}\d x'}+\mathcal{A}_{-}\e^{-\frac{\im}{\hbar}\int_{x_{0}}^{x}\abs{p(x')}\d x'}\right],
\end{equation}
kde $x_{0}$ je libovolný bod, od kterého se integruje (změna integrační meze $x_0$ pouze změní fázi vlnové funkce) a $\mathcal{A}_{\pm}$ jsou amplitudy obou komponent odpovídajícím vlně pohybující se podél osy $x$ a proti ose $x$ (amplitudy mohou být komplexní).
...
\begin{equation}
    \psi(x)=\frac{1}{\sqrt{\abs{p(x)}}}\left[\mathcal{B}_{+}\e^{\frac{1}{\hbar}\int_{x_{0}}^{x}\abs{p(x')}\d x'}+\mathcal{B}_{-}\e^{-\frac{1}{\hbar}\int_{x_{0}}^{x}\abs{p(x')}\d x'}\right].
\end{equation}