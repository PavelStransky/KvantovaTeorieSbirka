\subsection{Dvojitá symetrická jáma}
	...
	\item
		Srovnejte číselně s výsledkem příkladu~\ref{sec:QuarticOscillator}.	
	\end{enumerate}
...
\begin{solution}
	...
	\item
		...
		\item
			...
			\begin{equation}
				\psi_{\mathrm{I}}(x)=\frac{\mathcal{A}_{-}}{\sqrt{\abs{p(x)}}}\e^{-\frac{1}{\hbar}\int_{x}^{-b}\abs{p(x')}\d x'}=\lambda\e^{-I_{x}^{-b}}.
			\end{equation}
		...
		\item
			...
			K tomu se budou využívat součtové vztahy a další identity pro goniometrické funkce:
			\begin{align}
				\psi_{\mathrm{II}}(x)
		...
		\item
			Do oblasti III se přejde pomocí sešívacích vztahů~\eqref{eq:WKBConnectionUp1}--\eqref{eq:WKBConnectionUp2}.
			Vlnová funkce se následně hned upraví pro navázání do oblasti IV:
			\begin{align}
				\psi_{\mathrm{III}}(x)
		...
		\item
			Poslední navazování do oblasti V se provede opět pomocí vztahů~\eqref{eq:WKBConnectionUp1}---\eqref{eq:WKBConnectionUp2}:
			\begin{equation}
		...
		Předně se bude předpokládat, že prostupnost bariéry III je malá, takže $F\approx0$ a lze psát
		\begin{equation}
		...
		Kdyby nebylo tunelování mezi dvěma jamami, spektrum $E_{n}^{(0)}$ by na základě Bohrova-Sommerfeldova kvantování~\eqref{eq:BohrSommerfeld}\index{podmínka!Bohrova-Sommerfeldova kvantovací} bylo dáno rovnicí
		\begin{equation}
			\int_{a}^{b}\sqrt{2M\left[E_{n}^{(0)}-V(x)\right]}\,\d x=\pi\hbar\left(n+\frac{1}{2}\right).
		\end{equation}
		Ve skutečnosti však dochází k tunelování a spektrum je určeno rovnicí~\eqref{eq:DoubleWellE}.
		Budeme předpokládat, že hladina $E_{n}$ se nachází blízko hladiny $E_{n}^{(0)}$, tj.
		\begin{equation}
			E_{n}=E_{n}^{(0)}+\epsilon,
		\end{equation}
		kde $\epsilon\ll E_{n}^{(0)}$, a rozvineme levou stranu rovnice~\eqref{eq:DoubleWellE} okolo $E_{n}^{(0)}$:
		\begin{align}
	...
	\item
		...
		\end{equation}
		viz~\eqref{eq:HamiltonianQuarticOscillator}, takže se jedná o potenciál~\eqref{eq:DoubleWellPotential} s
		\begin{equation}
			g=2\sqrt{2}\,,\qquad x_{0}=\frac{1}{2},
		\end{equation}
		přičemž spektrum Hamiltoniánu bude navíc posunuté o $\frac{1}{16}$ energetické jednotky dolů.
		
		Pro dostatečně malé hodnoty $\hbar$ (v příkladu~\ref{sec:QuarticOscillator} se počítá s $\hbar=0.02$) vychází pro základní stav
		\begin{align}
			\Omega&=\frac{g x_{0}}{\sqrt{M}}=\sqrt{2}\approx1.4142\nonumber\\
			E_{0}&=\frac{1}{2}\hbar\Omega-\frac{1}{16}\approx-0.0484\nonumber\\
			\Delta E_{0}'&=1.81\cdot 10^{-6}
		\end{align}
		Naproti tomu na základě numerické diagonalizace matice v příkladu~\ref{sec:QuarticOscillator} je energie základího stavu a rozštěpení prvního dubletu
		\begin{align}
			E_{0}&=-0.0489 & \Delta E=1.86\cdot 10^{-6}.
		\end{align}
		Pro menší hodnoty $\hbar$ bychom dostali lepší shodu obou přístupů.
	\end{enumerate}
\end{solution}
