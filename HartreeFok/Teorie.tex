Systém $N$ nerozlišitelných částic interagujících nanejvýš dvoučásticovými interakcemi je popsaný Hamiltoniánem
    ...
    Základní stav se hledá ve tvaru Slaterova determinantu
    \begin{equation}
        \ket{\Psi}=\operatorconjugate{a}_{N}\dotsb\operatorconjugate{a}_{1}\ket{0}.
    \end{equation}
    variační metodou.
    To vede na \emph{Hartree-Fockovu rovnici} pro jednočásticové stavy $\ket{\phi_{n}}\equiv\operatorconjugate{a}_{n}\ket{0}$ a jednočásticové energie $\epsilon_{n}$ systému nerozlišitelných fermionů
    \begin{equation}
        \left(\operator{t}+\operator{v}^{(1)}+\operator{v}^{\text{HF}}\right)\ket{\phi_{n}}=\epsilon_{n}\ket{\phi_{n}},
        \label{eq:HFequation}
    \end{equation}
    kde $\operator{v}^{\text{HF}}$ je operátor Hartree-Fockova středního pole, pro který platí
    \begin{equation}
...
\sec{Řešení v zadané bázi}
    ...
    \end{equation}
    Hartree-Fockovu rovnici~\eqref{eq:HFequation} lze tedy přepsat do tvaru
    \begin{gather}
    ...
    \end{gather}
    což je (maticová) Hartree-Fockova rovnice v bázi $\mathcal{B}$.
    ...
    Hartree-Fockova rovnice~\eqref{eq:HFequationB} se řeší iterativní cestou.
    \begin{enumerate}
        \item Začne se s nějakým nástřelem matice $C_{nc}$.
        
        \item Z matice $C_{nc}$ se napočte Hartree-Fockovo pole $v_{ac}^{\text{HF}}$ podle vztahu~\eqref{eq:HFvHF} následně matice Hamiltoniánu
        \begin{equation}
        ...
        \item Vrátíme se k bodu 2, přičemž novou matici $C_{nc}$ použijeme jako vstup pro výpočet nového Hartree-Fockova pole.  
