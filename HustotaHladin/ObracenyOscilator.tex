\subsection{Obrácený oscilátor v jámě}
	...
	\begin{solution}
		Při výpočtu se vyjde ze vztahu~\eqref{eq:LevelDensity1D}.
		
		\begin{enumerate}
			\item $E<0$:		
				V tomto případě je (díkz sudosti potenciálu stačí počítat jen v oblasti $x>0$ a výsledek zdvojnásobit):
				\begin{align*}
				\Omega(E)
				...
				\end{align*}
				kde $-\frac{1}{2}ka^{2}\leq E\leq0$ a $\omega=\sqrt{k/m}$.
			...
			Analogickým postupem jako v případě záporné energie se dostane
			\begin{equation*}
				\rho(E)
