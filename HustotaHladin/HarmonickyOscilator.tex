...
\begin{solution}
    ...
    \Omega(E)
        &=\int\delta\left(E-\frac{1}{2M}p^{2}-\frac{1}{2}kq^{2}\right)\d q\d p
        &&\equationcomment{
                \pi=\sqrt{\frac{1}{2M}}\,p & \d p=\sqrt{2M}\d\pi\\
                \xi=\sqrt{\frac{k}{2}}\,q & \d q=\sqrt{\frac{2}{k}}\d\xi
            }\nonumber\\
        &=\sqrt{\frac{4M}{k}}\int\delta\left(E-\pi^{2}-\xi^{2}\right)\d\xi\d\pi
        &&\equationcomment{\textrm{Polární souřadnice} \\ r^{2}=\pi^{2}+\xi^{2}\\ \d\xi\d\pi=r\d r\d\varphi}\nonumber\\
        &=\frac{2}{\omega}\int_{0}^{2\pi}\d\varphi\int_{0}^{\infty}\delta\left(E-r^{2}\right)\,r\d r
        &&\equationcomment{\delta(E-r^{2})=\frac{1}{2\sqrt{E}}\left(\delta\left(r-\sqrt{E}\right)+\delta\left(r+\sqrt{E}\right)\right)}\nonumber\\
    ...
    \begin{flushleft}
    \emph{Ještě jiný způsob řešení:} % Martin Váňa, 2008
    \end{flushleft}
    ...
    \end{equation}
    Po dosazení do vztahu~\eqref{eq:PhaseSpaceVolume} vychází pro zadaný jednorozměrný harmonický oscilátor
    \begin{align}
        \Omega(E)
