...
\end{equation} 
kde poslední integrál je vlastně \emph{Lesbegueova míra}\index{míra!Lesbegueova} (\uv{objem}) energetické nadplochy fázového prostoru.  
...
\begin{align}
    \rho(E)
        &=\frac{1}{(2\pi\hbar)^{f}}\partialderivative{}{E}\int_{E>V(\vector{q})}\d^{f}\vector{q}\underbrace{\int_{0}^{\sqrt{2M[E-V(\vector{q})]}}p^{f-1}\d\Omega_{f}\d p}_{\text{objem }f\text{-rozměrné koule}}\nonumber\\
...
Fanoušci $\delta$ funkce mohou poslední vztah odvodit jinak.
...
\begin{subequations}
    ...
    \label{eq:Deltaf}        
\end{subequations}
a v rovnici~\eqref{eq:PhaseSpaceVolume} se provede integrace přes hybnost:
\begin{align}
    \Omega(E)
